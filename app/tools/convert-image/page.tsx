...
export default function ConvertImagePage() {
  ...
  return (
    <ToolPage title="Convert Image" description="Convert between JPG, PNG, WebP, GIF and more" icon={RefreshCw} color="bg-gradient-to-br from-cyan-500 to-cyan-700">
      {!file ? (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
          <FileUpload onFiles={onFiles} accept={{ 'image/*': ['.jpg','.jpeg','.png','.webp','.gif','.bmp','.tiff'] }} multiple={false} label="Select image to convert" />
        </div>
      ) : (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 space-y-5">
          ...
          <div>
            <div className="flex items-center gap-3 mb-3">
              <span className="text-sm font-semibold text-gray-500">{srcExt}</span>
              <span className="text-gray-400">→</span>
              <span className="text-sm font-bold text-cyan-600 uppercase">{format}</span>
            </div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">Convert To</label>
            <div className="grid grid-cols-3 gap-2">
              {FORMATS.map(f => (
                <button key={f} type="button" onClick={() => setFormat(f)}
                  className={`py-3 rounded-xl border-2 text-sm font-semibold uppercase tracking-wide transition ${format === f ? 'border-cyan-500 bg-cyan-50 text-cyan-700' : 'border-gray-200 text-gray-600 hover:border-cyan-200'}`}>
                  .{f}
                </button>
              ))}
            </div>
